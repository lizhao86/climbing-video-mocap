...
# 蹲跳（Lunge）/ 外撐（Stemming）都试过、都砍了（2026-07-18，S4 收尾抽验）：
# - 外撐 STEM_WIDTH=1.1bl 报 121 个，抽查全是正常宽站或左右踝认错（span 3bl 物理不可能）；
# - 蹲跳「双手事件重叠」收紧上下限后抽验仍 3/3 假（逮到的是快速连续换手和远景噪声）。
# 共同教训：**素材里没有正例的动作不上规则**——阈值无从标定，产出必是误报
# （与 GAP_PRESS_LO 同理）。等有内角/含跳素材再回来标定，实现留在 git 历史。
...
def main():
    ...
    # 蹲跳（Lunge）已砍（2026-07-18）：「双手换点事件重叠」这个信号实践里逮到的是
    # 快速连续换手（始终有一手在点上）和远景抖动噪声，不是 dyno——抽验 3/3 假
    # （六盘水 2 个：人趴墙上纹丝没动；IMG_6411@16.9：连续快速出手）。收紧
    # overlap/disp 上下限后仍然全假。这批素材里没有人真跳过 → 零正例，
    # 与外撐同理不硬造判据。真要做 dyno 检测得等有含跳跃的素材再标定。

    moves_out.sort(key=lambda m: m["t_s"])

    # ── 攀爬窗过滤：metrics_v2 存在时，把起攀前/完攀后的「动作」全部丢掉────────
    # 走近岩壁、下墙、拍粉这些墙下行为会造出假动作（首版 IMG_6411@0.1s 报了个
    # 2.8s 的「蹲跳」，其实人还没上墙）。
    mv2_path = os.path.join(d, f"{base}_metrics_v2.json")
    if os.path.exists(mv2_path):
        C = json.load(open(mv2_path, encoding="utf-8"))["completion"]
        w0, w1 = C["start_s"], C["end_s"]
        n_before = len(moves_out)
        moves_out = [m for m in moves_out if w0 <= m["t_s"] <= w1]
        if n_before != len(moves_out):
            print(f"   攀爬窗 [{w0:.1f}, {w1:.1f}]s 过滤掉窗外 {n_before - len(moves_out)} 条")
