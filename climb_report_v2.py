...
# ── 卡住判据（2026-07-20 重做，不再依赖换点事件计数）─────────────────────
# 旧判据是「同一高度箱内 move 段 ≥3」。move 段建在换点事件上，于是出手阈值一动，
# 卡点就跟着漂——2026-07-20 把 EV_MIN_NET 从 0.50 提到 0.70 之后，IMG_6152 那个
# 真卡住段直接消失了。根子是**两个用途要相反的敏感度**：数出手要滤掉微调，
# 判卡住恰恰要靠微调。
#
# 新判据直接读重心轨迹，老板的原话就是判据本身：
#   「9-13 秒算卡住，我在找中心，避免自己开门，你看我重心移动，且脚下也有动作」
#   → 重心在动（路径长）+ 高度不涨（净上升小）+ **期间没换成点**（事件数=0）
# 第三条是关键：横移和大动作同样满足前两条，但它们有换点事件。事件在这里当
# **排除条件**，不是触发条件。
# IMG_6152 标定：全片只命中 8.7-12.7s，老板口述 9-13s，无误报。
STUCK_WIN_S    = 4.00   # 滑窗长度（秒）
STUCK_MAX_GAIN = 0.40   # 窗内净上升上限（躯干长），超过说明在往上走
STUCK_MIN_PATH = 1.00   # 窗内重心路径长下限（躯干长），低于说明是静止而非「在磨」
STUCK_MAX_EV   = 0      # 窗内允许的换点事件数，>0 说明在做动作不是卡住
STUCK_STEP_S   = 0.50   # 扫描步长
...
# ── 流畅度（2026-07-18 新增，依据深度调研：docs/2026-07-18-指标价值review与路线决策.md）──
# 两个指标都只用 2D 髋轨迹——单目最可信的信号区，完全不碰 3D 关节角度。
# - GIE 几何熵 = ln(2L/c)，L=髋轨迹长，c=轨迹凸包周长（Cordier 1993 起，多实验室
#   复现 30 年；原文明说单机位可行）。轨迹越绕，熵越高；高手更低、随练习下降。
# - 归一化 jerk（Seifert 2014 用 IMU 验证过指标概念）：轨迹平滑度，路线难找时升高。
#   ⚠️ 三阶导对噪声极敏感，必须先重平滑；我们的数值口径是自己的（视频轨迹≠IMU），
#   绝对值与文献不可比。
# 两个指标**跨路线不可比**（依赖路线形状），只用于同线复爬对比 / 同人跨时间趋势——
# 展示层必须带这句话，不给假解读。
FLUENCY_SMOOTH_S = 0.5   # jerk 前的轨迹平滑窗（秒）。太小=噪声爆炸，太大=抹掉真实变向
...
def hull_perimeter(pts):
    """2D 点集凸包周长（Andrew 单调链，纯 numpy，不引 scipy）。pts 需无 NaN。"""
    if len(pts) < 3:
        return float("nan")
...
def main():
    ...
    # 3b 卡住型：重心在动 + 高度不涨 + 期间没换成点（判据来历见文件顶部旋钮区）
    all_ev = S.get("events") or []
    com_x = trunk[:, 0] / body_scale        # trunk 已是肩髋中点；除以 body_scale 归一化
    com_y = trunk[:, 1] / body_scale
    hits = []
    tt = t_start
    while tt + STUCK_WIN_S <= t_end:
        m = (t >= tt) & (t <= tt + STUCK_WIN_S)
        if m.sum() >= 10:
            hh = height[m]
            gain_w = float(hh[-1] - hh[0])
            path_w = float(np.nansum(np.hypot(np.diff(com_x[m]), np.diff(com_y[m]))))
            n_ev = sum(1 for e in all_ev if tt <= e.get("start_s", -1) <= tt + STUCK_WIN_S)
            if gain_w < STUCK_MAX_GAIN and path_w > STUCK_MIN_PATH and n_ev <= STUCK_MAX_EV:
                hits.append((tt, tt + STUCK_WIN_S, gain_w, path_w))
        tt += STUCK_STEP_S
    # 相邻命中窗合并成一段
    for a, b_, g_, p_ in hits:
        if crux and crux[-1].get("kind") == "repeat" and a <= crux[-1]["_end"]:
            c_ = crux[-1]
            c_["_end"] = max(c_["_end"], b_)
            c_["_gain"] = max(c_["_gain"], g_)
            c_["_path"] = max(c_["_path"], p_)
        else:
            crux.append({"kind": "repeat", "_start": a, "_end": b_, "_gain": g_, "_path": p_})
    for c_ in crux:
        if c_.get("kind") != "repeat" or "_start" not in c_:
            continue
        a, b_ = c_.pop("_start"), c_.pop("_end")
        g_, p_ = c_.pop("_gain"), c_.pop("_path")
        c_["t"] = round((a + b_) / 2, 2)
        c_["detail"] = (f"{a:.1f}-{b_:.1f}s 重心一直在动（走了 {p_:.1f} 躯干的路），"
                        f"高度却只涨了 {g_:.2f} 躯干，期间一个点都没换成——在找重心，没找到")
    ...
    # ── 汇总 metrics_v2.json ─────────────────────────────────
    # ── 6.5 流畅度（GIE + 归一化 jerk，只用 2D 髋轨迹，窗口=起攀→完攀）────────
    hp_win = hp[i_start:i_end + 1]
    ok_hp = ~np.isnan(hp_win).any(axis=1)
    hp_c = hp_win[ok_hp] / body_scale          # 身长归一，跨人可比一点
    fluency = {"gie": None, "log_norm_jerk": None,
               "path_len_bl": None, "hull_perim_bl": None,
               "note": "跨路线不可比（依赖路线形状）；用于同线复爬对比或同人跨时间趋势"}
    if len(hp_c) > int(2 * fps):               # 至少 2 秒有效轨迹才算
        L = float(np.sum(np.linalg.norm(np.diff(hp_c, axis=0), axis=1)))
        c = hull_perimeter(hp_c)
        if np.isfinite(c) and c > 1e-6:
            fluency["gie"] = round(float(np.log(2 * L / c)), 3)
            fluency["path_len_bl"] = round(L, 2)
            fluency["hull_perim_bl"] = round(c, 2)
        # jerk：重平滑后三阶差分。平滑窗按秒换算成帧，避免跨 fps 口径漂移
        w = max(13, int(FLUENCY_SMOOTH_S * fps) | 1)
        hs = smooth2d(hp_win, w)[ok_hp] / body_scale
        dtm = float(np.nanmedian(np.diff(t[i_start:i_end + 1])))
        if len(hs) > 3 and dtm > 0 and L > 1e-6:
            j = np.gradient(np.gradient(np.gradient(hs, axis=0), axis=0), axis=0) / dtm ** 3
            jj = np.nansum(np.linalg.norm(j, axis=1) ** 2) * dtm
            T = len(hs) * dtm
            # Hogan-Sternad 无量纲化：×T⁵/L²。取 ln 压量级（原始值跨素材差几个数量级）
            fluency["log_norm_jerk"] = round(float(np.log(np.sqrt(0.5 * jj * T ** 5 / L ** 2))), 2)
    ...
    M = {
        ...
        "params": {"START_RISE": START_RISE, "ENGAGE_HOLD_S": ENGAGE_HOLD_S,
                   ...
                   "PREP_CRUX_K": PREP_CRUX_K,
                   "STUCK_WIN_S": STUCK_WIN_S, "STUCK_MAX_GAIN": STUCK_MAX_GAIN,
                   "STUCK_MIN_PATH": STUCK_MIN_PATH, "STUCK_MAX_EV": STUCK_MAX_EV,
                   "VIS_GAP_FILL_S": VIS_GAP_FILL_S,
                   ...
                   "GAP_PRESS_LO": GAP_PRESS_LO, "FLUENCY_SMOOTH_S": FLUENCY_SMOOTH_S},
    }
