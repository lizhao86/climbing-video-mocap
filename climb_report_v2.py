...
def main():
    ...
    for b, ss in sorted(bins.items()):
        if len(ss) >= BIN_MOVE_N:
            crux.append({"t": round(float(np.mean([x["start_s"] for x in ss])), 2),
                         "kind": "repeat",
                         "detail": f"高度 {round(b*HEIGHT_BIN,1)}-{round((b+1)*HEIGHT_BIN,1)}bl "
                                   f"区间内反复出手 {len(ss)} 次（{ss[0]['start_s']}-{ss[-1]['end_s']}s）"})
    # 3c 与 v1 crux 互证
    v1_crux = v1.get("crux_times_s", [])
    for c in crux:
        hit = [x for x in v1_crux if abs(x - c["t"]) < CRUX_MATCH_S]
        c["v1_confirmed"] = bool(hit)
        c["v1_crux_t"] = hit[0] if hit else None
    crux.sort(key=lambda c: c["t"])
    n_confirmed = sum(1 for c in crux if c["v1_confirmed"])
    ...
    M = {
        ...
        "crux": {"n": len(crux), "n_v1_confirmed": n_confirmed,
                 "v1_crux_times_s": v1_crux, "items": crux},
        "ratios": ratios,
    ...
    takeaways.append(f"完攀用时 {climb_s}s（起攀 {round(t_start,1)}s → 完攀 {round(t_end,1)}s），"
                     f"净上升 {M['completion']['net_gain_bl']} 身长。"
                     + ("" if top_held else "⚠️ 顶点没有明显停留，完攀时刻取的是重心最高帧，可能是没到顶或直接下撤。"))
    if prep_vals:
    ...
    if crux:
        takeaways.append(f"识别到 {len(crux)} 处难点，其中 {n_confirmed} 处与 v1 的吃力点双命中（标"
                         f"「确认难点」，可信度最高）。")
    else:
        takeaways.append("没有明显难点：全程没有异常长的停顿，也没有在同一高度反复试探。")
    ...
    crux_rows = "".join(
        f'<tr><td>{c["t"]}s</td>'
        f'<td>{"犹豫" if c["kind"]=="hesitation" else "反复试探"}</td>'
        f'<td>{c["detail"]}</td>'
        f'<td>{"<b style=color:#ff6b6b>确认难点</b>" if c["v1_confirmed"] else "疑似"}</td></tr>'
        for c in crux) or '<tr><td colspan="4" style="color:#9aa0aa">未识别到难点</td></tr>'
...
{card(f"{len(crux)} <span style='font-size:15px;color:#9aa0aa'>({n_confirmed} 确认)</span>", "难点数", "与 v1 吃力点双命中=确认", "orange")}
...
<h2>难点明细</h2><table><tr><th>时刻</th><th>类型</th><th>依据</th><th>置信</th></tr>{crux_rows}</table>
<div class="note">「确认难点」= v2a 的节奏异常与 v1 报告卡的吃力点（重心加速度 + 肢体速度 + 关节极端屈曲）在 {CRUX_MATCH_S}s 内双命中。单独命中标「疑似」。v1 吃力点时刻：{v1_crux or "（无 v1 metrics.json，未做互证）"}</div>
...
<div class="note"><b>数据口径：</b>本报告卡不依赖动作识别，只统计节奏。分段来自 climb_segments.py（锚点位移法）。长度/速度以「身长」为单位（尺度无关），与 v1 报告卡同口径。单摄像头 + 相机静止假设，单目对深度不敏感。阈值参数全部集中在 climb_report_v2.py 顶部，验收后可调；本次取值见 metrics_v2.json 的 params 字段。</div>
    ...
    print(f"   难点 {len(crux)} 处（v1 双命中 {n_confirmed}）")
