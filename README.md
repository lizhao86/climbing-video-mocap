...
- **S2 全身动作分段**：已完成。`climb_segments.py` 锚点位移法——手/脚末端净位移 ≥0.7 躯干长判"真换点"（`EV_MIN_NET`，2026-07-20 用逐动作口述从 0.5 重标），输出 move/static 段 + 肢体换点事件两层；`climb_segments_review.py` 生成烧字幕审阅视频供人眼验收。
...
身高体重存在 `账本配置.json`。跟 Claude 说一次，它会写进去，之后不用再管。
身高用来把爬升换算成米数（乘躯干长≈0.29×身高），不填就只显示相对量「躯干」。
注意米数偏保守——单目仰拍会低估高墙，抱石矮墙基本准，绳索高墙约差一半。
...
排除。当前素材 4 条常规线（IMG_6152/6321/6411/6942，逐条特性见
[CLAUDE.md](CLAUDE.md) 的验证素材表）+ 1 条远景顶绳（六盘水斗牛，走 ROI 流程）。
（IMG_6417/6418 于 2026-07-20 删除——不是老板本人的攀爬。）
