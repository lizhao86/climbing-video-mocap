...
- **旅程账本（主干门面）**：可用。`攀岩账本.html` 一页看累计数字（线路数/爬升/
  ...
  每条记录点进对应报告卡。**填写攀岩信息**：双击 `打开账本填写.bat`，打开的就是
  账本页的可编辑版——每条记录行内直接填难度/完攀/岩馆，底部填身高，保存自动更新。
  日期自动读视频拍摄时间（文件时间是拷贝时间，不可靠）。
...
```bash
双击 打开账本填写.bat        # 账本页（可编辑版）：看数字 + 行内填难度/完攀/岩馆/身高
# 或者直接双击 攀岩账本.html  # 纯看（静态版），不能填
```

分析完新视频后重跑两条命令账本就会收录：

```bash
python3 climb_journal.py        # 扫 素材/*/数据/ + 线路.json → 素材/journal.json
python3 climb_journal_card.py   # → 攀岩账本.html
```
...
| `climb_journal_edit.py` + `打开账本填写.bat` | 账本填写服务（本机 8765）：行内填难度/完攀/岩馆/身高 |
