...
- **顶点保持要报实际时长而非只报 True/False**：IMG_6152 保持 1.37s vs 阈值 1.5s，其实是
  ...
  报告卡按"≥60% 阈值"给不同措辞。
- **rest 分支在三条素材上全部未触发**：静止段"较直那条臂"要么 <150°（弯着扛），要么虽
  达 155-166° 但持续 <2s（爬得快，不挂）。即**从不直臂休息**——真实发现，也是最有价值的
  教练结论（前臂先泵的直接原因）。副作用：休息点质量（第 6 项）实现了但未被任何素材验证，
  需一条含真实休息的视频。注：抱石线路短而连续，真休息本就是先锋/长线才有的东西，
  若后续素材仍全是抱石，此项可能长期无法验证——届时考虑放宽 REST_MIN_S 或改口径。
