...
| S3 | 节奏统计报告 v2a | 🔄 待老板验收 | 2026-07-16 | 六项指标全部实现，climb_report_v2.py + 报告卡 + metrics_v2.json 跑通两条新视频。**IMG_6942 自验证通过**（抽帧核对）：起攀 1.7s / 完攀 31.4s / 29.73s，与画面吻合；19-24s 的"卡住"由弯臂 3.41s + 同高度反复出手 + 4.58s 长停顿三信号互证，抽帧确认人确实停在同一高度弯臂硬扛。**三项待老板拍板**：① rest 全程为 0（静止段肘角中位数最高仅 149°，从未直臂挂过）——数字是真的，但休息点质量指标因此未被验证；② v2a 难点与 v1 crux 双命中 0，两者测的是正交的东西（v1=动作剧烈，v2a=卡住不动），"双命中=确认难点"的设计前提可能不成立；③ IMG_6947 素材被污染（13-16s 路人挡镜头致 MediaPipe 跟错人、30s 后人爬出画面顶部），不适合做验证素材 |
| S4 | 特征+规则匹配 | ⬜ 待做 | | |
| S5 | 视觉判定环节 | ⬜ 待做 | | |
| S6 | 标注界面+回流 | ⬜ 待做 | | |
...
- **⚠️「双命中=确认难点」的前提存疑**：IMG_6942 上 v2a 难点与 v1 crux 双命中 **0**。
  抽帧核对确认 **v2a 判对了**（19-24s 人确实卡在同一高度弯臂硬扛），v1 完全没发现该段。
  两者测的是正交的东西：v1 靠加速度/肢体速度找"动作剧烈"的瞬间，v2a 找"卡住不动"的时刻，
  而真正的难点常常是后者。互证机制保留但不应作为可信度主判据——待老板拍板。
- **rest 分支在两条素材上均未触发**：静止段"较直那条臂"的肘角中位数最高仅 149°，
  从未达到直臂线 150°——即全程没有一次直臂休息（真实发现，也是最有价值的教练结论）。
  副作用：休息点质量（第 6 项指标）实现了但未被任何素材验证，需一条含真实休息的视频。
